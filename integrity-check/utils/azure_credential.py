...
Every consumer in this container (SQL via pyodbc, Service Bus receiver +
publisher, Azure OpenAI, Blob storage) shares this single instance:
  ...
  * `managed_identity_client_id` is declared here and NOWHERE else, so the
    ...
    IMDS which identity to request — that was the receiver's original bug.)
