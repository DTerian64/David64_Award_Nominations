...
def handle(payload: dict) -> None:
    ...
    # The beneficiary is told they received an award (amount included). When the
    # tenant has opted in, the certificate PDF is attached — generated by the
    # backend at approval time and downloaded here. A missing certificate never
    # blocks the email; we just send it without the attachment.
    if status in APPROVED_STATUSES and details.get("beneficiary_email"):
        attachments = None
        ...
        if cert_cfg["enabled"] and cert_cfg["attach_to_beneficiary"]:
            ...
            if pdf:
                ...
                logger.warning(
                    "Certificate attachment enabled but PDF unavailable — "
                    "sending beneficiary email without attachment",
